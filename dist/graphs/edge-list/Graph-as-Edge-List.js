 ...
 */
function hasUndirectedPath(graph, start, end) {
    ...
    while (queue.length) {
        ...
        if (visited.has(current))
            continue; // don't revisit (no need to add children again too)
        // Check for end of path
        if (current === end)
            return true; // @todo Object.is and ===
...
*/
function shortestPathSizeByNumEdges(graph, start, end) {
    ...
    if (adjList.has(start)) {
        ...
        const stack = []; // @todo handle start differently bc sets of children.
 ...
 * @param end
 */
function shortestPathSizeByWeights(graph, start, end) {
 ...
 * @param end
 */
function calculateAllPathSizes(edgeList, start, end) {
    // @todo ensure that start and end are in the graph
    // find start
...
//   }
// }
//# sourceMappingURL=Graph-as-Edge-List.js.map