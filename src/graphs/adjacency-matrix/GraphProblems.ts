...
const isLand = (val: string): boolean => val === 'L';
...
function islandCount(grid: string[][]) {
  ...
  const visitedLocations: boolean[][] = new Array(grid.length); // m * n size array, start with all unvisited
  // Fill visitedLocations with rows/arrays of false (making it a 2d array)
  for (let i = 0; i < numRows; i++) {
    const subArray: boolean[] = new Array(numCols).fill(false);
  ...
  for (let r = 0; r < grid.length; r++) {
    ...
    for (let c = 0; c < row.length; c++) {
       ...
       * 
       */
      function getAdjacentLocations(r: number, c: number, rMin: number, rMax: number, cMin: number, cMax: number) {
      ...
      if (isLand(gridItem) && !isVisited(r, c)) {
        ...
        const locQueue: [number, number][] = [[r, c]]; // enqueue the grid item's location in the location queue
        ...
        while (locQueue.length) {
          ...
          getAdjacentLocations(r, c, rMin, rMax, cMin, cMax).forEach(([row, col]: [number, number]) => {
 ...
 */
function minIslandSize(grid: string[][]) {
  let minIsland = 0;
  ...
  const isVisited = (r: number, c: number) => visitedLocations[r][c] === true;
  const markVisited = (r: number, c: number) => visitedLocations[r][c] = true;
  ...
  const visitIsland = (r: number, c: number) => {

  };
  const visitConnectedLand = (r: number, c: number, islandSize: number) => {

    return islandSize + 1;
  };

  // Check neighbors: above, below, to left, and to right. Must be within range.
  const getNeighborLocations = (r: number, c: number, rowMax: number, colMax: number, rowMin: number = 0, colMin: number = 0) => {
    const neighbors = [];
