import type { IVertex, AdjacencyList } from "../../types/graph-types"
...
export class DAG implements DAG {
  public adjacencyList: AdjacencyList = new Map();
...
function connectedComponentsCount(graph: AdjacencyList): number {
  ...
  for (const [vertex] of graph) {
    ...
    forUntil(graph, vertex, (v) => {
      ...
      if (!isVisited(v)) {
        markVisited(v);
        if (!foundConnectedComponent) foundConnectedComponent = true;
      }
    }, isVisited);
    if (foundConnectedComponent) count++;
  }
 ...
 * @returns The size of the largest component
 */
function largestComponentSize(graph: AdjacencyList): number {
  let largestSize = 0;
  const visited: Set<IVertex> = new Set();
  const isVisited = (v: IVertex): boolean => visited.has(v);
  const markVisited = (v: IVertex): void => { visited.add(v); }
